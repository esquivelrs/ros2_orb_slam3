...
public:
    ...
    scale_factor(0.0)
  {
    ...
    image_sub = create_subscription<sensor_msgs::msg::Image>(
      "camera/color/image_raw", 10, std::bind(
        &ImuMonoRealSense::image_callback, this,
        _1), image_options);
    ...
    pose_delta_sub = create_subscription<ros2_orb_slam3::msg::PoseDelta>(
      "pose_delta", 10, std::bind(
        &ImuMonoRealSense::pose_delta_callback, this,
        _1), pose_delta_options);

    // create clients
    apriltag_client = create_client<ros2_orb_slam3::srv::AprilTagDetection>(
      "apriltag_detection",
      rmw_qos_profile_services_default,
      apriltag_callback_group);
...
private:
  ...
  void save_map_to_csv(vector<ORB_SLAM3::MapPoint *> map_points)
  {
    ...
    std::string map_file_name = std::string(PROJECT_PATH) + "/maps/" + file_name + "_" +
      time_string + "map" + ".csv";
    std::string unscaled_map_file_name = std::string(PROJECT_PATH) + "/unscaled_maps/" + file_name +
    ...
    if (map_file.is_open() && unscaled_map_file.is_open()) {
      ...
      for (auto & map_point : map_points) {
        ...
        map_file << std::fixed << std::setprecision(3) << pos[0] * scale_factor << "," <<
          pos[1] * scale_factor << "," << pos[2] * scale_factor << std::endl;
        unscaled_map_file << std::fixed << std::setprecision(3) << pos[0] << "," << pos[1] << "," <<
          pos[2] << std::endl;
      }
      map_file.close();
    } else {
  ...
  void image_callback(const sensor_msgs::msg::Image::SharedPtr msg)
  {
    ...
    if (sensor_type_param == "monocular") {
      ...
      RCLCPP_INFO_STREAM(get_logger(), "Tcw: \n" << Tcw.matrix());
      RCLCPP_INFO_STREAM(get_logger(), "count2 " << count2);
      // only on the first slam iteration
      if (!Tcw.matrix().isIdentity() && count2 == 0) {

        RCLCPP_INFO_STREAM(get_logger(), "HEREasadsa");
        ...
        //       if (std::sqrt(
        //           std::pow(
        //             center.x - key_points.at(i).pt.x,
        //             2) + std::pow(center.y - key_points.at(i).pt.y, 2)) < eps)
        //       {
        ...
        //         cv::line(
        //           frame, key_points.at(i).pt, cv::Point(center.x, center.y),
        //           cv::Scalar(255, 0, 0), 2);
        //         cv::imshow("frame", frame);
        //         cv::waitKey(1);
        //         RCLCPP_INFO_STREAM(
        //           get_logger(), "keypoint: " << key_points.at(
        //             i).pt.x << ", " << key_points.at(i).pt.y);
  ...
  void imu_callback(const sensor_msgs::msg::Imu msg)
  {
    ...
    ORB_SLAM3::IMU::Point imu_meas(
      static_cast<float>(msg.linear_acceleration.x), static_cast<float>(msg.linear_acceleration.y),
